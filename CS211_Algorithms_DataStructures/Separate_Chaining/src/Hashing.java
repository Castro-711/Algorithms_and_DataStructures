...
public class Hashing 
{
	...
	static LinkedList<String>[] linkHashTable = new LinkedList[size];
...
//fill the hash table so that every slot contains a space
      ...
      for(int i = 0; i < linkHashTable.length; i++)
      {
    	  linkHashTable[i] = new LinkedList<String>();
      }
//loads up the file
...
//the whole thing is added together and then moduloed to create a hash table index
    	 ...
    	 for(int i = 0; i < word.length(); i++)
    	 {
    		 ...
    		 // holds the current i value as not to get zero
    		 // although it works fine with
    		 BigInteger iVal = new BigInteger("" + i);
    		
    		 BigInteger power = new BigInteger("" + curLetterAscii.multiply(iVal.add(BigInteger.ONE)));
...
//for example, you could just add up all of the letters in the word
    	 ...
    	 //hashTable[getHashKey(word)].addLast(word);
    	 
    	 int start = 0;
    	 BigInteger largeNumber = new BigInteger("1" );
    	 int curPower = 42;
    	 
    	 while(start < word.length())
    	 {
    		 BigInteger letterAscii = new BigInteger("" + (int) word.charAt(start));
    		 largeNumber = largeNumber.add(letterAscii);
    		 
    		 if(start == word.length() - 1)
    		 {
    			 largeNumber = largeNumber.pow(curPower);
...
//print out the information for the last 1,000 words only, otherwise it takes quite long and gets annoying
     ...
     public static void fillDoubleHash(){
        ...
        for(int i=0; i<array.length;i++){
           ...
           while(linkHashTable[index].getFirst()!=null){
